...
if __name__ == '__main__':
    ...
    while (R.get_all_possible_moves() != []):
        if R.color == 1:
            ...
            # print(R.get_all_possible_moves())
            print("Player O: Possible moves")
            for possible_move in R.get_all_possible_moves():
                print(str(R.num2tuple(possible_move))+"-->"+str(possible_move))
            x = True
            while x==True:
                m = int(input("Enter your move: "))
                if m in R.get_all_possible_moves():
                    x = False
                else:
            ...
            Tsta = time.time()
            m = UCT(rootstate = R, itermax = difficulty)
            Tend = time.time()
            print('Player x\nTime taken:'+str(Tend-Tsta))
